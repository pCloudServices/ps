...
$actual = Test-WSMan -ComputerName "localhost" -ErrorAction SilentlyContinue
if ($actual){
    try 
    {
...
if ($WinRMService)
{
    ...
    Try{
      ...
      Catch{
        if ($Error[0].Exception.Message -like "*The config setting CredSSP cannot be changed because is controlled by policies*")
            {
...
else {$errorMsg = "Verify WinRM service is running"}
...
{
$actual = Test-Path -Path $profile.$profiles
    if ($actual -eq $true)
    {
...
$TunnelIP = Read-Host "Please enter your TunnelConnector IP Address (Leave empty if you don't have it)"
    If ($TunnelIP) 
    {
    $actual = Test-NetConnection -ComputerName $TunnelIP -Port 443 -WarningVariable TunnelIPerror | select -ExpandProperty "TcpTestSucceeded"
    $result = $actual
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAZuou7JSMqfgzY
# k+Ifr3g7WWSLG8KzcANHNIR0wkMvHKCCDnUwggROMIIDNqADAgECAg0B7l8Wnf+X
...
# AYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAvBgkqhkiG9w0BCQQxIgQgOrcSc5Z9O0Jk
# dqvwud9td8lXcetmPrPi5kkw6qgLO9swDQYJKoZIhvcNAQEBBQAEggEAlTaDQsIL
# ecpUJTjMtQDeSFKt9sAwuedhSuWc3VlkQJtVFG9HQM2Okr++JI80uh40h9svK+D3
# zdyShLLEWPbw9YSyjmqrdKDybm6N3ZNajkE2kd3fwOBh5G6XDzx/q7N6PoW1LJoK
# HZn9anYYLzcV0vc2sCJxP3qysuGJpRhYnhQ8NRVtRSSovSEVABb73kfUpm8HppES
# zb06PLZ81KCbman0qddrsiOI1nDXKKLUZySV3BcbWFzq6AVdkKF/t4KnbpDCEL54
# pKkLQM8V+0YfSn70DqgDiENEqDsAHfcFeIG1aSChur1zCelfeHKfbht7VJGOBu/c
# k0LkyhBZ6iHiW6GCDiwwgg4oBgorBgEEAYI3AwMBMYIOGDCCDhQGCSqGSIb3DQEH
# AqCCDgUwgg4BAgEDMQ0wCwYJYIZIAWUDBAIBMIH/BgsqhkiG9w0BCRABBKCB7wSB
# 7DCB6QIBAQYLYIZIAYb4RQEHFwMwITAJBgUrDgMCGgUABBTxF6UxotcOJ6l22otf
# sgzGQKJHRAIVAM3AVS6To5TCfcFa6CsMnoHGtLRSGA8yMDIwMDkwMTA3MDA1MVow
...
# CQMxDQYLKoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTIwMDkwMTA3MDA1MVow
# LwYJKoZIhvcNAQkEMSIEIII2iW2Qa5P3TEs+dYe9GSofX0P2A/HVqeLwj446f/GX
# MDcGCyqGSIb3DQEJEAIvMSgwJjAkMCIEIMR0znYAfQI5Tg2l5N58FMaA+eKCATz+
# 9lPvXbcf32H4MAsGCSqGSIb3DQEBAQSCAQASQvXgOzLFmRXyLOglk+cQk5rsiIqy
# Xf+8Qt0P2UABH+/cAN6Miyrdsewlntt1fYqtz0vP5k4puwrDgNF1mpAW4NrW3752
# 6XYnhp1XAhnQ2XDGWr/qhzxphFQnkmC1J24UN9k/3TVJF9F2neirHBTvCkSIjr/f
# gBxoLZJf3UYV/WfY/tZUeuEiqFeAfXSl/2o/M1os931Ad0TKzYfBbXnXPXWq2IGs
# SbC+lAct2kAr8aQL6mOCFFv2DzgHJIoV3CaXq04SRl1B9x+EPWa9tQUTtl93ytwf
# a0HDKUOGAbnnfu2u4JffQyss5d6PraPYltlj89A0E3NFiHcZe+nZ/SJK
